...
/**
 * Logs all MINA protocol events.  Each event can be tuned to use a different level based on the
 * user's specific requirements.  Methods are in place that allow the user to use either the get or
 * set method for each event and pass in the {@link IoEventType} and the {@link LogLevel}.
 * <p>
 * By default, all events are logged to the {@link LogLevel#INFO} level except {@link
 * IoFilterAdapter#exceptionCaught(IoFilter.NextFilter, IoSession, Throwable)}, which is logged to
 * {@link LogLevel#WARN}.
 *
...
public class LoggingFilter extends IoFilterAdapter {
    ...
    /**
     * Log if the logger and the current event log level are compatible. We log a formated message
     * and its parameters.
     *
     * @param eventLevel the event log level as requested by the user
