...
/**
 * An {@link IoFilter} implementation used to buffer outgoing {@link WriteRequest} almost like what
 * {@link BufferedOutputStream} does. Using this filter allows to be less dependent from network
 * latency. It is also useful when a session is generating very small messages too frequently and
 * consequently generating unnecessary traffic overhead.
 * <p>
 * Please note that it should always be placed before the {@link ProtocolCodecFilter} as it only
 * handles {@link WriteRequest}'s carrying {@link IoBuffer} objects.
 *
...
public final class BufferedWriteFilter extends IoFilterAdapter {
    ...
    /**
     * The map that matches an {@link IoSession} and it's {@link IoBuffer} buffer.
     */
    private final LazyInitializedCacheMap<IoSession, IoBuffer> buffersMap;

    /**
     * Default constructor. Sets buffer size to {@link #DEFAULT_BUFFER_SIZE} bytes. Uses a default
    ...
    /**
     * Constructor which sets buffer size to <code>bufferSize</code>.Uses a default instance of
     ...
     * {@inheritDoc}
     *
     * @throws Exception if <code>writeRequest.message</code> isn't an {@link IoBuffer} instance.
     */
    @Override
    ...
    /**
     * Writes <code>data</code> {@link IoBuffer} to the <code>buf</code> {@link IoBuffer} which
     * buffers write requests for the
    ...
    /**
     * Internal method that actually frees the {@link IoBuffer} that contains the buffered data that
