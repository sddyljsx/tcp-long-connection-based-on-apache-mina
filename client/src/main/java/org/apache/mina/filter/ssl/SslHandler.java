 ...
 * <p/>
 * Each connection has a SSLEngine that is used through the lifetime of the connection. We allocate
 * buffers for use as the outbound and inbound network buffers. These buffers handle all of the
 * intermediary data for the SSL connection. To make things easy, we'll require outNetBuffer be
 * completely flushed before trying to wrap any more data.
 * <p/>
 * This class is not to be used by any client, it's closely associated with the SSL Filter. None of
 * its methods are public as they should not be used by any other class but from the SslFilter
