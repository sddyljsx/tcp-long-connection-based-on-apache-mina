...
/**
 * A filter that forwards I/O events to {@link Executor} to enforce a certain thread model while
 * allowing the events per session to be processed simultaneously. You can apply various thread
 * model by inserting this filter to a {@link IoFilterChain}.
 *
 * <h2>Life Cycle Management</h2>
 * <p>
 * Please note that this filter doesn't manage the life cycle of the {@link Executor}. If you
 * created this filter using {@link #ExecutorFilter(Executor)} or similar constructor that accepts
 * an {@link Executor} that you've instantiated, you have full control and responsibility of
 * managing its life cycle (e.g. calling {@link ExecutorService#shutdown()}.
 * <p>
 * If you created this filter using convenience constructors like {@link #ExecutorFilter(int)}, then
 * you can shut down the executor by calling {@link #destroy()} explicitly.
 *
 * <h2>Event Ordering</h2>
 * <p>
 * All convenience constructors of this filter creates a new {@link OrderedThreadPoolExecutor}
 * instance.  Therefore, the order of event is maintained like the following:
 * <ul>
...
public class ExecutorFilter extends IoFilterAdapter {
    ...
    /**
     * A set of flags used to tell if the Executor has been created in the constructor or passed as
     * an argument. In the second case, the executor state can be managed.
     **/
    ...
    /**
     * (Convenience constructor) Creates a new instance with a new {@link
     * OrderedThreadPoolExecutor}, no thread in the pool, and a maximum of 16 threads in the pool.
     * All the event will be handled by this default executor.
     */
    ...
    /**
     * (Convenience constructor) Creates a new instance with a new {@link
     * OrderedThreadPoolExecutor}, no thread in the pool, but a maximum of threads in the pool is
     * given. All the event will be handled by this default executor.
     *
    ...
    /**
     * (Convenience constructor) Creates a new instance with a new {@link
     * OrderedThreadPoolExecutor}, a number of thread to start with, a maximum of threads the pool
     * can contain. All the event will be handled by this default executor.
     *
    ...
    /**
     * Creates a new instance of ExecutorFilter. This private constructor is called by all the
     * public constructor.
     *
     * @param executor           The underlying {@link Executor} in charge of managing the Thread
     *                           pool.
    ...
    /**
     * Shuts down the underlying executor if this filter hase been created via a convenience
     * constructor.
     */
    @Override
    public void destroy() {
