...
/**
 * An {@link IoFilter} that sends a keep-alive request on {@link IoEventType#SESSION_IDLE} and sends
 * back the response for the sent keep-alive request.
 *
 * <h2>Interference with {@link IoSessionConfig#setIdleTime(IdleStatus, int)}</h2>
 * <p>
 * This filter adjusts <tt>idleTime</tt> of the {@link IdleStatus}s that this filter is interested
 * in automatically (e.g. {@link IdleStatus#READER_IDLE} and {@link IdleStatus#WRITER_IDLE}.)
 * Changing the <tt>idleTime</tt> of the {@link IdleStatus}s can lead this filter to a unexpected
 * behavior. Please also note that any {@link IoFilter} and {@link IoHandler} behind {@link
 * KeepAliveFilter} will not get any {@link IoEventType#SESSION_IDLE} event.  To receive the
 * internal {@link IoEventType#SESSION_IDLE} event, you can call {@link #setForwardEvent(boolean)}
 * with <tt>true</tt>.
 *
 * <h2>Implementing {@link KeepAliveMessageFactory}</h2>
 * <p>
 * To use this filter, you have to provide an implementation of {@link KeepAliveMessageFactory},
 * which determines a received or sent message is a keep-alive message or not and creates a new
 * keep-alive message:
 *
 * <table border="1">
 * <tr>
...
public class KeepAliveFilter extends IoFilterAdapter {
    ...
    /**
     * Returns <tt>true</tt> if and only if this filter forwards a {@link IoEventType#SESSION_IDLE}
     * event to the next filter. By default, the value of this property is <tt>false</tt>.
     */
    ...
    /**
     * Sets if this filter needs to forward a {@link IoEventType#SESSION_IDLE} event to the next
     * filter. By default, the value of this property is <tt>false</tt>.
     */
