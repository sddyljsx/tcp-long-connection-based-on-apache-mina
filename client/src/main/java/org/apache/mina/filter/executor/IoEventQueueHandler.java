...
/**
 * Listens and filters all event queue operations occurring in {@link OrderedThreadPoolExecutor} and
...
public interface IoEventQueueHandler extends EventListener {
    ...
    /**
     * Returns <tt>true</tt> if and only if the specified <tt>event</tt> is allowed to be offered to
     * the event queue.  The <tt>event</tt> is dropped if <tt>false</tt> is returned.
     */
    boolean accept(Object source, IoEvent event);

    /**
     * Invoked after the specified <tt>event</tt> has been offered to the event queue.
     */
    void offered(Object source, IoEvent event);

    /**
     * Invoked after the specified <tt>event</tt> has been polled from the event queue.
     */
