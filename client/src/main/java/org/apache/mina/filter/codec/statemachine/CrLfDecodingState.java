...
/**
 * {@link DecodingState} which decodes a single <code>CRLF</code>. If it is found, the bytes are
 * consumed and <code>true</code> is provided as the product. Otherwise, read bytes are pushed back
 * to the stream, and <code>false</code> is provided as the product. Note that if we find a CR but
 * do not find a following LF, we raise an error.
 *
...
public abstract class CrLfDecodingState implements DecodingState {
     ...
     * @param out       the current {@link ProtocolDecoderOutput} used to write decoded messages.
