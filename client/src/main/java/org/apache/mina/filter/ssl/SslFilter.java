...
/**
 * An SSL filter that encrypts and decrypts the data exchanged in the session. Adding this filter
 * triggers SSL handshake procedure immediately by sending a SSL 'hello' message, so you don't need
 * to call {@link #startSsl(IoSession)} manually unless you are implementing StartTLS (see below).
 * If you don't want the handshake procedure to start immediately, please specify {@code false} as
 * {@code autoStart} parameter in the constructor.
 * <p>
 * This filter uses an {@link SSLEngine} which was introduced in Java 5, so Java version 5 or above
 * is mandatory to use this filter. And please note that this filter only works for TCP/IP
 * connections.
 * <p>
 *
...
public class SslFilter extends IoFilterAdapter {
    ...
    /**
     * A session attribute key that makes next one write request bypass this filter (not encrypting
     * the data).  This is a marker attribute, which means that you can put whatever as its value.
     * ({@link Boolean#TRUE} is preferred.)  The attribute is automatically removed from the session
     * attribute map as soon as {@link IoSession#write(Object)} is invoked, and therefore should be
     * put again if you want to make more messages bypass this filter.  This is especially useful
     * when you implement StartTLS.
     */
    public static final AttributeKey DISABLE_ENCRYPTION_ONCE = new AttributeKey(SslFilter.class, "disableOnce");

    /**
     * A session attribute key that makes this filter to emit a {@link
     * IoHandler#messageReceived(IoSession, Object)} event with a special message ({@link
     * #SESSION_SECURED} or {@link #SESSION_UNSECURED}). This is a marker attribute, which means
     * that you can put whatever as its value. ({@link Boolean#TRUE} is preferred.)  By default,
     * this filter doesn't emit any events related with SSL session flow control.
     */
    public static final AttributeKey USE_NOTIFICATION = new AttributeKey(SslFilter.class, "useNotification");

    /**
     * A session attribute key that should be set to an {@link InetSocketAddress}. Setting this
     * attribute causes {@link SSLContext#createSSLEngine(String, int)} to be called passing the
     * hostname and port of the {@link InetSocketAddress} to get an {@link SSLEngine} instance. If
     * not set {@link SSLContext#createSSLEngine()} will be called.<br/> Using this feature {@link
     * SSLSession} objects may be cached and reused when in client mode.
     *
    ...
    /**
     * A special message object which is emitted with a {@link IoHandler#messageReceived(IoSession,
     * Object)} event when the session is secured and its {@link #USE_NOTIFICATION} attribute is
     * set.
     */
    ...
    /**
     * A special message object which is emitted with a {@link IoHandler#messageReceived(IoSession,
     * Object)} event when the session is not secure anymore and its {@link #USE_NOTIFICATION}
     * attribute is set.
     */
    ...
    /**
     * Creates a new SSL filter using the specified {@link SSLContext}. The handshake will start
     * immediately.
     */
     ...
     * <code>autostart</code> flag is set to <code>true</code>, the handshake will start
     * immediately.
     */
    ...
    /**
     * (Re)starts SSL session for the specified <tt>session</tt> if not started yet. Please note
     * that SSL session is automatically started by default, and therefore you don't need to call
     * this method unless you've used TLS closure.
     *
    ...
    /**
     * An extended toString() method for sessions. If the SSL handshake is not yet completed, we
     * will print (ssl) in small caps. Once it's completed, we will use SSL capitalized.
     */
    ...
    /**
     * Returns <tt>true</tt> if and only if the specified <tt>session</tt> is encrypted/decrypted
     * over SSL/TLS currently. This method will start to return <tt>false</tt> after TLS
     * <tt>close_notify</tt> message is sent and any messages written after then is not going to get
     * encrypted.
     */
    ...
    /**
     * Returns <tt>true</tt> if the engine is set to use client mode when handshaking.
     */
    ...
    /**
     * Returns <tt>true</tt> if the engine will <em>require</em> client authentication. This option
     * is only useful to engines in the server mode.
     */
    ...
    /**
     * Configures the engine to <em>require</em> client authentication. This option is only useful
     * for engines in the server mode.
     */
    ...
    /**
     * Returns <tt>true</tt> if the engine will <em>request</em> client authentication. This option
     * is only useful to engines in the server mode.
     */
    ...
    /**
     * Configures the engine to <em>request</em> client authentication. This option is only useful
     * for engines in the server mode.
     */
    ...
    /**
     * A message that is sent from {@link SslFilter} when the connection became secure or is not
     * secure anymore.
     *
     * @author <a href="http://mina.apache.org">Apache MINA Project</a>
     */
