...
/**
 * An abstract implementation of {@link IoProcessor} which helps transport developers to write an
 * {@link IoProcessor} easily. This class is in charge of active polling a set of {@link IoSession}
...
public abstract class AbstractPollingIoProcessor<S extends AbstractIoSession> implements IoProcessor<S> {
    ...
    /**
     * The maximum loop count for a write operation until {@link #write(AbstractIoSession, IoBuffer,
     * int)} returns non-zero value. It is similar to what a spin lock is for in concurrency
     * programming. It improves memory utilization and write throughput significantly.
     */
    ...
    /**
     * Create an {@link AbstractPollingIoProcessor} with the given {@link Executor} for handling
     * I/Os events.
     *
    ...
    /**
     * Compute the thread ID for this class instance. As we may have different classes, we store the
     * last ID number into a Map associating the class name to the last assigned ID.
     *
     * @return a name for the current thread, based on the class name and an incremental value,
     * starting at 1.
     */
    ...
    /**
     * Dispose the resources used by this {@link IoProcessor} for polling the client connections.
     * The implementing class doDispose method will be called.
     *
    ...
    /**
     * Reads a sequence of bytes from a {@link IoSession} into the given {@link IoBuffer}. Is called
     * when the session was found ready for reading.
     *
    ...
    /**
     * Write a sequence of bytes to a {@link IoSession}, means to be called when a session was found
     * ready for writing.
     *
    ...
    /**
     * Write a part of a file to a {@link IoSession}, if the underlying API isn't supporting system
     * calls like sendfile(), you can throw a {@link UnsupportedOperationException} so the file will
     * be send using usual {@link #write(AbstractIoSession, IoBuffer, int)} call.
     *
     * @param session the session to write
    ...
    /**
     * Starts the inner Processor, asking the executor to pick a thread in its pool. The Runnable
     * will be renamed
     */
    ...
    /**
     * In the case we are using the java select() method, this method is used to trash the buggy
     * selector and create a new one, registring all the sockets on it.
     *
    ...
    /**
     * Check that the select() has not exited immediately just because of a broken connection. In
     * this case, this is a standard case, and we just have to loop.
     *
    ...
    /**
     * Loops over the new sessions blocking queue and returns the number of sessions which are
     * effectively created
     *
    ...
    /**
     * Process a new session : - initialize it - create its chain - fire the CREATED listeners if
     * any
     *
    ...
    /**
     * The main loop. This is the place in charge to poll the Selector, and to process the active
     * sessions. It's done in - handle the newly created sessions -
     */
