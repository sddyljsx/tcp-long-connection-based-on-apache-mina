...
public abstract class IoBuffer implements Comparable<IoBuffer> {
    ...
    /**
     * Returns <tt>true</tt> if and only if a direct buffer is allocated by default when the type of
     ...
     * specified. The default value is <tt>false</tt>.
     */
     ...
     * optimal memory usage and performance. If it is greater than or equal to {@link
     * Integer#MAX_VALUE}, it returns {@link Integer#MAX_VALUE}. If it is zero, it returns zero.
     */
    ...
    /**
     * Declares this buffer and all its derived buffers are not used anymore so that it can be
     * reused by some {@link IoBufferAllocator} implementations. It is not mandatory to call this
     * method, but you might want to invoke this method for maximum performance.
     */
    ...
    /**
     * returns <tt>true</tt> if and only if this buffer is derived from other buffer via {@link
     * #duplicate()}, {@link #slice()} or {@link #asReadOnlyBuffer()}.
     */
    ...
    /**
     * Returns the minimum capacity of this buffer which is used to determine the new capacity of
     * the buffer shrunk by {@link #compact()} and {@link #shrink()} operation. The default value is
    ...
    /**
     * Changes the capacity and limit of this buffer so this buffer get the specified
     * <tt>expectedRemaining</tt> room from the specified
    ...
    /**
     * Returns the position of the current mark. This method returns <tt>-1</tt> if no mark is set.
     */
    ...
    /**
     * Clears this buffer and fills its content with <tt>NUL</tt>. The position is set to zero, the
     * limit is set to the capacity, and the mark is discarded.
     */
    public abstract IoBuffer sweep();

    /**
     * double Clears this buffer and fills its content with <tt>value</tt>. The position is set to
    ...
    /**
     * Returns an {@link InputStream} that reads the data from this buffer. {@link
     * InputStream#read()} returns <tt>-1</tt> if the buffer position reaches to the limit.
     */
     ...
     * as a terminator.
     * <p>
     * Please note that this method doesn't terminate with <code>NUL</code> if the input string is
     * longer than <tt>fieldSize</tt>.
     *
     ...
     * specified <code>decoder</code> and returns it. This method is a shortcut for
     * <tt>getPrefixedString(2, decoder)</tt>.
     */
    ...
    /**
     * Writes the content of <code>in</code> into this buffer as a string which has a 16-bit length
     * field before the actual encoded string, using the specified <code>encoder</code>. This method
     * is a shortcut for
    ...
    /**
     * Writes the content of <code>in</code> into this buffer as a string which has a 16-bit length
     * field before the actual encoded string, using the specified <code>encoder</code>.
     *
     * @param prefixLength the length of the length field (1, 2, or 4)
     ...
     * not that using this method can allow DoS (Denial of Service) attack in case the remote peer
     * sends too big data length value. It is recommended to use {@link #prefixedDataAvailable(int,
     * int)} instead.
     *
    ...
    /**
     * Reads an int from the buffer and returns the correlating enum constant defined by the
     * specified enum type.
     *
     * @param <E>       The enum type to return
     * @param enumClass The enum's class object
     ...
     * first entry in the specified enum and each subsequent bit maps to each subsequent bit as
     * mapped to the subsequent enum value.
     * </p>
     *
