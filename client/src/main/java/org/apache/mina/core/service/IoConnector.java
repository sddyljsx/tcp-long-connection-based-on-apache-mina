 ...
 * example.
 * <p>
 * You should connect to the desired socket address to start communication, and then events for
...
public interface IoConnector extends IoService {
    ...
    /**
     * Returns the default remote address to connect to when no argument is specified in {@link
     * #connect()} method.
     */
    SocketAddress getDefaultRemoteAddress();

    /**
     * Sets the default remote address to connect to when no argument is specified in {@link
     * #connect()} method.
     */
    ...
    /**
     * Connects to the {@link #setDefaultRemoteAddress(SocketAddress) default remote address} and
     * invokes the <code>ioSessionInitializer</code> when the IoSession is created but before {@link
     * IoHandler#sessionCreated(IoSession)} is invoked.  There is <em>no</em> guarantee that the
     * <code>ioSessionInitializer</code> will be invoked before this method returns.
     *
     * @param sessionInitializer the callback to invoke when the {@link IoSession} object is
     *                           created
     ...
     *
     * @return the {@link ConnectFuture} instance which is completed when the connection attempt
     * initiated by this call succeeds or fails.
     */
    ConnectFuture connect(SocketAddress remoteAddress);

    /**
     * Connects to the specified remote address and invokes the <code>ioSessionInitializer</code>
     * when the IoSession is created but before {@link IoHandler#sessionCreated(IoSession)} is
     * invoked.  There is <em>no</em> guarantee that the <code>ioSessionInitializer</code> will be
    ...
    /**
     * Connects to the specified remote address binding to the specified local address and and
     * invokes the <code>ioSessionInitializer</code> when the IoSession is created but before {@link
     * IoHandler#sessionCreated(IoSession)} is invoked.  There is <em>no</em> guarantee that the
     * <code>ioSessionInitializer</code> will be invoked before this method returns.
     *
