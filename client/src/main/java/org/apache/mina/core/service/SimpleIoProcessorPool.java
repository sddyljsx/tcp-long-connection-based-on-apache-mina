...
/**
 * An {@link IoProcessor} pool that distributes {@link IoSession}s into one or more {@link
 * IoProcessor}s. Most current transport implementations use this pool internally to perform better
 * in a multi-core environment, and therefore, you won't need to use this pool directly unless you
 * are running multiple {@link IoService}s in the same JVM.
 * <p>
 * If you are running multiple {@link IoService}s, you could want to share the pool among all
 * services.  To do so, you can create a new {@link SimpleIoProcessorPool} instance by yourself and
 * provide the pool as a constructor parameter when you create the services.
 * <p>
 * This pool uses Java reflection API to create multiple {@link IoProcessor} instances. It tries to
 * instantiate the processor in the following order:
 * <ol>
...
public class SimpleIoProcessorPool<S extends AbstractIoSession> implements IoProcessor<S> {
    ...
    /**
     * Creates a new instance of SimpleIoProcessorPool with a defined number of IoProcessors in the
     * pool
     *
     * @param processorType The type of IoProcessor to use
    ...
    /**
     * Find the processor associated to a session. If it hasen't be stored into the session's
     * attributes, pick a new processor and stores it.
     */
    @SuppressWarnings("unchecked")
