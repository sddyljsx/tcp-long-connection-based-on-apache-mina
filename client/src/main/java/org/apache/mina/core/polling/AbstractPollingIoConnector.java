...
/**
 * A base class for implementing client transport using a polling strategy. The underlying sockets
 * will be checked in an active loop and woke up when an socket needed to be processed. This class
 * handle the logic behind binding, connecting and disposing the client sockets. A {@link Executor}
 * will be used for running client connection, and an {@link AbstractPollingIoProcessor} will be
 * used for processing connected client I/O operations like reading, writing and closing.
 * <p>
 * All the low level methods for binding, connecting, closing need to be provided by the subclassing
 * implementation.
 *
...
public abstract class AbstractPollingIoConnector<T extends AbstractIoSession, H> extends AbstractIoConnector {
    ...
    /**
     * Constructor for {@link AbstractPollingIoConnector}. You need to provide a default session
     * configuration, a class of {@link IoProcessor} which will be instantiated in a {@link
     * SimpleIoProcessorPool} for better scaling in multiprocessor systems. The default pool size
     * will be used.
     *
     * @param sessionConfig  the default configuration for the managed {@link IoSession}
     * @param processorClass a {@link Class} of {@link IoProcessor} for the associated {@link
     *                       IoSession} type.
    ...
    /**
     * Constructor for {@link AbstractPollingIoConnector}. You need to provide a default session
     * configuration, a default {@link Executor} will be created using {@link
     * Executors#newCachedThreadPool()}.
     *
     * @param sessionConfig the default configuration for the managed {@link IoSession}
     * @param processor     the {@link IoProcessor} for processing the {@link IoSession} of this
     *                      transport, triggering events to the bound {@link IoHandler} and
     *                      processing the chains of {@link IoFilter}
    ...
    /**
     * Constructor for {@link AbstractPollingIoConnector}. You need to provide a default session
     * configuration and an {@link Executor} for handling I/O events. If null {@link Executor} is
     * provided, a default one will be created using {@link Executors#newCachedThreadPool()}.
     *
     * @param sessionConfig the default configuration for the managed {@link IoSession}
     * @param executor      the {@link Executor} used for handling asynchronous execution of I/O
    ...
    /**
     * Constructor for {@link AbstractPollingIoAcceptor}. You need to provide a default session
     * configuration and an {@link Executor} for handling I/O events. If null {@link Executor} is
     * provided, a default one will be created using {@link Executors#newCachedThreadPool()}.
     *
     * @param sessionConfig    the default configuration for the managed {@link IoSession}
     ...
     *                         transport, triggering events to the bound {@link IoHandler} and
     *                         processing the chains of {@link IoFilter}
     * @param createdProcessor tagging the processor as automatically created, so it will be
    ...
    /**
     * Destroy the polling system, will be called when this {@link IoConnector} implementation will
     * be disposed.
     *
    ...
    /**
     * Connect a newly created client socket handle to a remote {@link SocketAddress}. This
     * operation is non-blocking, so at end of the call the socket can be still in connection
     * process.
     *
    ...
    /**
     * Finish the connection process of a client socket after it was marked as ready to process by
     * the {@link #select(int)} call. The socket will be connected or reported as connection
     * failed.
     *
     * @param handle the client socket handle to finish to connect
    ...
    /**
     * Create a new {@link IoSession} from a connected socket client handle. Will assign the created
     * {@link IoSession} to the given {@link IoProcessor} for managing future I/O events.
     *
     ...
     * failed to connect). All the client socket descriptors processed need to be returned by {@link
     * #selectedHandles()}
     *
    ...
    /**
     * {@link Iterator} for the set of client sockets found connected or failed to connect during
     * the last {@link #select(int)} call.
     *
