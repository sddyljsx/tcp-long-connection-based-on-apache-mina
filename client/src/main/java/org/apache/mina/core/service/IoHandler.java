...
public interface IoHandler {
     ...
     * #sessionCreated(IoSession)}.  The biggest difference from {@link #sessionCreated(IoSession)}
     * is that it's invoked from other thread than an I/O processor thread once thread model is
     * configured properly.
     */
    ...
    /**
     * Invoked with the related {@link IdleStatus} when a connection becomes idle. This method is
     * not invoked if the transport type is UDP; it's a known bug, and will be fixed in 2.0.
     */
    void sessionIdle(IoSession session, IdleStatus status) throws Exception;

    /**
     * Invoked when any exception is thrown by user {@link IoHandler} implementation or by MINA.  If
     * <code>cause</code> is an instance of {@link IOException}, MINA will close the connection
     * automatically.
     */
