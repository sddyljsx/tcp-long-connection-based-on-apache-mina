...
public class Socks4LogicHandler extends AbstractSocksLogicHandler {
    ...
    /**
     * Encode a SOCKS4/SOCKS4a request and writes it to the next filter so it can be sent to the
     * proxy server.
     *
     * @param nextFilter the next filter
    ...
    /**
     * Handle incoming data during the handshake process. Should consume only the handshake data
    ...
    /**
     * Handle a SOCKS4/SOCKS4a response from the proxy server. Test the response buffer reply code
     * and call {@link #setHandshakeComplete()} if access is granted.
     *
