...
public class NTLMUtilities implements NTLMConstants {
     ...
     * in the data that defines some data with a variable length. This allows to have a semi-fixed
     ...
     * @param allocated    the allocated space for the security buffer (should be greater or equal
     *                     to <code>length</code>
     * @param bufferOffset the offset from the main array where the currently defined security
    ...
    /**
     * Writes the Windows OS version passed in as three byte values (majorVersion.minorVersion.buildNumber)
     * to the given byte array at <code>offset</code>.
     *
    ...
    /**
     * Tries to return a valid OS version on Windows systems. If it fails to do so or if we're
     * running on another OS then a fake Windows XP OS version is returned because the protocol uses
     * it.
     *
    ...
    /**
     * Writes a security buffer and returns the pointer of the position where to write the next
     * security buffer.
     *
     ...
     *
     * @param msg      the type 2 message byte array
     * @param msgFlags the flags if null then flags are extracted from the type 2 message
     * @return the target name
     * @throws UnsupportedEncodingException if unable to use the needed UTF-16LE or ASCII charsets
     */
    ...
    /**
     * Prints to the {@link PrintWriter} the target information block extracted from the type 2
     * message.
     *
     ...
     * @see <a href="http://davenport.sourceforge.net/ntlm.html#theType3Message">NTLM message
     * type</a>
     */
