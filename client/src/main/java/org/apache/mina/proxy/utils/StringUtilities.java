...
public class StringUtilities {
    ...
    /**
     * A directive is a parameter of the digest authentication process. Returns the value of a
     * directive from the map. If mandatory is true and the value is null, then it throws an {@link
     ...
     * @throws AuthenticationException if mandatory is true and if directivesMap.get(directive) ==
     *                                 null
     */
    ...
    /**
     * Copy the directive to the {@link StringBuilder} if not null. (A directive is a parameter of
    ...
    /**
     * Copy the directive from the source map to the destination map, if it's value isn't null. (A
     * directive is a parameter of the digest authentication process.)
     *
    ...
    /**
     * Parses digest-challenge string, extracting each token and value(s). Each token is a
     * directive.
     *
     ...
     * @throws SaslException if either the key or the value is null or if the key already has a
     *                       value.
     */
    ...
    /**
     * Is character a linear white space ? LWS            = [CRLF] 1*( SP | HT ) Note that we're
     * checking individual bytes instead of CRLF
     *
    ...
    /**
     * Used to convert username-value, passwd or realm to 8859_1 encoding if all chars in string are
     * within the 8859_1 (Latin 1) encoding range.
     *
    ...
    /**
     * Returns the value of the named header. If it has multiple values then an {@link
     ...
     * @param singleValued if true and the map already contains one value then it is replaced by the
     *                     new value. Otherwise it simply adds a new value to this multi-valued
     *                     header.
     */
