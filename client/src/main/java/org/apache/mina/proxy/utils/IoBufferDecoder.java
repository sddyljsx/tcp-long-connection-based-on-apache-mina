...
/**
 * IoBufferDecoder.java - Handles an {@link IoBuffer} decoder which supports two methods : - dynamic
 * delimiter decoding - fixed length content reading
 *
...
public class IoBufferDecoder {
    ...
    /**
     * Creates a new instance that uses specified <tt>delimiter</tt> byte array as a message
     * delimiter.
     *
    ...
    /**
     * Sets the the length of the content line to be decoded. When set, it overrides the dynamic
     * delimiter setting and content length method will be used for decoding on the next decodeOnce
     * call. The default value is <tt>-1</tt>.
     *
    ...
    /**
     * Dynamically sets a new delimiter. Next time {@link #decodeFully(IoBuffer)} will be called it
     * will use the new delimiter. Delimiter matching is reset only if <tt>resetMatchCount</tt> is
     * true but decoding will continue from current position.
     * <p>
    ...
    /**
     * Will return null unless it has enough data to decode. If <code>contentLength</code> is set
     * then it tries to retrieve <code>contentLength</code> bytes from the buffer otherwise it will
     * scan the buffer to find the data <code>delimiter</code> and return all the data and the
     * trailing delimiter.
     *
