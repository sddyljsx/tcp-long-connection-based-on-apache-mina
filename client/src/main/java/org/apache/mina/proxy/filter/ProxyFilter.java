...
/**
 * ProxyFilter.java - Proxy {@link IoFilter}. Automatically inserted into the {@link IoFilter} chain
 * by {@link ProxyConnector}. Sends the initial handshake message to the proxy and handles any
 * response to the handshake. Once the handshake has completed and the proxied connection has been
...
public class ProxyFilter extends IoFilterAdapter {
    ...
    /**
     * Called before the filter is added into the filter chain. Checks if chain already holds an
     * {@link ProxyFilter} instance.
     *
     * @param chain      the filter chain
     * @param name       the name assigned to this filter
     * @param nextFilter the next filter
     * @throws IllegalStateException if chain already contains an instance of {@link ProxyFilter}
     */
    @Override
    ...
    /**
     * Called when the filter is removed from the filter chain. Cleans the {@link ProxyIoSession}
    ...
    /**
     * Called when an exception occurs in the chain. A flag is set in the {@link ProxyIoSession}
     * session's instance to signal that handshake failed.
     *
    ...
    /**
     * Actually write data. Queues the data up unless it relates to the handshake or the handshake
     * is done.
     *
     * @param nextFilter      the next filter in filter chain
     * @param session         the session object
    ...
    /**
     * Filter handshake related messages from reaching the messageSent callbacks of downstream
     * filters.
     *
     * @param nextFilter   the next filter in filter chain
     * @param session      the session object
    ...
    /**
     * Called when the session is created. Will create the handler able to handle the {@link
     * ProxyIoSession#getRequest()} request stored in the session. Event is stored in an {@link
     * IoSessionEventQueue} for later delivery to the next filter in the chain when the handshake
     * would have succeed. This will prevent the rest of the filter chain from being affected by
     * this filter internals.
     * <p>
     * Please note that this event can occur multiple times because of some http proxies not
     * handling keep-alive connections thus needing multiple sessions during the handshake.
     *
     * @param nextFilter the next filter in filter chain
     * @param session    the session object
     */
    ...
    /**
     * Event is stored in an {@link IoSessionEventQueue} for later delivery to the next filter in
     * the chain when the handshake would have succeed. This will prevent the rest of the filter
     * chain from being affected by this filter internals.
     *
     * @param nextFilter the next filter in filter chain
     * @param session    the session object
     */
