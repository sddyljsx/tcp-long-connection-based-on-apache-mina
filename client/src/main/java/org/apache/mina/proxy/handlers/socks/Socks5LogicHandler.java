...
public class Socks5LogicHandler extends AbstractSocksLogicHandler {
     ...
     * @param request the socks proxy request data
     * @return the encoded buffer
     * @throws UnsupportedEncodingException if request's hostname charset can't be converted to
     *                                      ASCII.
     */
     ...
     *
     * @param request the socks proxy request data
     * @return the encoded buffer, if null then authentication step is over and handshake process
     * can jump immediately to the next step without waiting for a server reply.
    ...
    /**
     * Encodes a SOCKS5 request and writes it to the next filter so it can be sent to the proxy
     * server.
     *
     * @param nextFilter the next filter
