...
/**
 * AbstractHttpLogicHandler.java - Base class for HTTP proxy {@link AbstractProxyLogicHandler}
 * implementations. Provides HTTP request encoding/response decoding functionality.
 *
...
public abstract class AbstractHttpLogicHandler extends AbstractProxyLogicHandler {
    ...
    /**
     * A flag that indicates that this is a HTTP/1.1 response with chunked data.and that some chunks
     * are missing.
     */
    private boolean hasChunkedData;

    /**
     * A flag that indicates that some chunks of data are missing to complete the HTTP/1.1
     * response.
     */
    private boolean waitingChunkedData;

    /**
     * A flag that indicates that chunked data has been read and that we're now reading the
     * footers.
     */
    private boolean waitingFooters;

    /**
     * Contains the position of the entity body start in the <code>responseData</code> {@link
     * IoBuffer}.
     */
    private int entityBodyStartPosition;

    /**
     * Contains the limit of the entity body start in the <code>responseData</code> {@link
     * IoBuffer}.
     */
    ...
    /**
     * Handles incoming data during the handshake process. Should consume only the handshake data
     * from the buffer, leaving any extra data in place.
     *
    ...
    /**
     * Calls writeRequest0(NextFilter, HttpProxyRequest) to write the request. If needed a
     * reconnection to the proxy is done previously.
     *
     * @param nextFilter the next filter
    ...
    /**
     * Method to reconnect to the proxy when it decides not to maintain the connection during
     * handshake.
     *
     * @param nextFilter the next filter
     * @param request    the http request
