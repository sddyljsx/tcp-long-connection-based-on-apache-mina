 ...
 * <p>
 * Using this interface has the advantage that it can be automatically determined when a component
 * <code>ByteArray</code> can no longer be written to, and thus components can be automatically
 * flushed. This makes it easier to use pooling for underlying <code>ByteArray</code>s.
 * <p>
 * By providing an appropriate <code>Expander</code> it is also possible to automatically add more
 * backing storage as more data is written. <br/><br/> TODO: Get flushing working.
 *
...
public class CompositeByteArrayRelativeWriter extends CompositeByteArrayRelativeBase implements IoRelativeWriter {
    ...
    /**
     * Expands the supplied {@link CompositeByteArray} by the number of bytes provided in the
     * constructor
     */
     ...
     *
     * @param cba       The CompositeByteArray to use to back this class
     * @param expander  The expander.  Will increase the size of the internal ByteArray
     * @param flusher   Flushed the ByteArray when necessary
     * @param autoFlush Should this class automatically flush?
     */
